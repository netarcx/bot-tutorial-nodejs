...
console.log(tba.getAwardHistoryForTeam(1816, callback(err, award_history))) {
...
function respond() {
  ...
  if(request.text && botRegex.test(request.text)) {
    ...
    postMessage("api trial");
    this.res.end();
  }
    else if(request.text && botRegexEV.test(request.text)) {
    this.res.writeHead(200);
    postMessage("The Green Machine last played the Gitch and WON! ");
    this.res.end();
  }
   else {
